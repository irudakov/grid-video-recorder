...
import io.nirvagi.VideoHttpDownloader;
import io.nirvagi.config.RuntimeConfig;
import io.nirvagi.node.task.config.TaskDescriptions;
import io.nirvagi.utils.json.JsonCodec;
import io.nirvagi.utils.json.JsonParserWrapper;
import io.nirvagi.utils.threads.video.VideoRecordingThreadPool;
...
public class VideoRecorder extends ExecuteOSTask{
    ...
    protected String buildUrlToVideo(String fileName) {
        try {
            ...
            .scheme("http")
            .host(RuntimeConfig.getHostIp())
            //Set up fixed node port so far
            .port(5555)
            //.setPort(RuntimeConfig.getGridExtrasPort())
            .setPathSegment(1,VideoHttpDownloader.GET_VIDEO_FILE_ENDPOINT + "/" + fileName);
